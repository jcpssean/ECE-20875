...
def gaus_mixture(data, n_components):
    ...
    gm = GaussianMixture(n_components[0], random_state=0).fit(data) # fill in 
    best_bic = bic = gm.bic(data) # fill in
    ...
    for k in n_components:
        ...
        gm_temp = GaussianMixture(k, random_state=0).fit(data)
        # calculate BIC
        bic_temp = gm_temp.bic(data)
